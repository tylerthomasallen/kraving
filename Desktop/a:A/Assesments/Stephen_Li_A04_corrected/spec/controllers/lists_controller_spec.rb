...
RSpec.describe ListsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    context 'when logged out' do
      ...
      it 'redirects to the login page' do
        post :create, params: { list: { name: 'Here\'s a task', description: 'do it' } }
        expect(response).to redirect_to(new_session_url)
      end
    end
  end
  ...
  describe 'PATCH #update' do
    ...
    context 'when logged in as the list\'s owner' do
      ...
      it 'allows users to update their lists' do
        patch :update, params: { id: jill_list.id, list: {name: 'Jill Task'} }

        updated_list = List.find(jill_list.id)
        expect(updated_list.name).to eq('Jill Task')
      end
    end
