...
def subwords(word, dictionary)
  new_word = substrings(word)
  new_word.select { |w| dictionary.include?(w)}
end
...
class Array
  def my_map(&prc)
    ans = []
    my_each {|el| ans << prc.call(el)}
    ans
  end

  def my_select(&prc)
    ans = []
    my_each {|el| ans << el if prc.call(el)}
    ans
  end
...
def concatenate(strings)
  strings.inject("") do |total , strings|
    total + strings
  end
end
